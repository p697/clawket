...
import { CodexRelay } from '../../packages/bridge-runtime/src/codex/relay';
import type { CodexService } from '../../packages/bridge-runtime/src/codex/service';
...
it('pairs an isolated Codex deployment and routes requests, streams, offline errors and reconnects without an OpenClaw challenge', async () => {
  const state = await mkdtemp(join(tmpdir(), 'clawket-codex-relay-'));
  ...
  let registry: CompatWranglerDevProcess | undefined, relay: CompatWranglerDevProcess | undefined, owner: CodexRelay | undefined;
  const sockets: WebSocket[] = [];
  try {
    registry = await CompatWranglerDevProcess.start({ cwd: process.cwd(), configPath: 'apps/relay-registry/wrangler.codex.example.toml', port: registryPort, inspectorPort: inspector1, persistencePath: state,
    ...
    relay = await CompatWranglerDevProcess.start({ cwd: process.cwd(), configPath: 'apps/relay-worker/wrangler.codex.example.toml', port: relayPort, inspectorPort: inspector2, persistencePath: state,
    ...
    const pair = await post('/v1/pair/register', { displayName: 'Codex fixture', preferredRegion: 'us' });
    const qrPayload = JSON.stringify({ v: 2, k: 'cp', b: 'codex', s: registry.baseUrl, g: pair.gatewayId, a: pair.accessCode });
    ...
    class Service extends EventEmitter {
      conversation = this;
      async request(frame: any) { return frame.method === 'health' ? { backend: 'codex', protocol: 1 } : { marker: frame.params?.marker }; }
    }
    const service = new Service();
    owner = new CodexRelay(service as unknown as CodexService, { relayUrl, gatewayId: pair.gatewayId, relaySecret: pair.relaySecret, invitation: { ...invitation, codeKeyHex, qrPayload, attempts: 0 } }, () => {});
    ...
    const claim = await post('/v1/pair/claim', { gatewayId: pair.gatewayId, accessCode: pair.accessCode, clientLabel: 'Codex phone' });
    ...
    expect(await first.nextJson(frame => frame.id === 'one')).toMatchObject({ ok: true, payload: { backend: 'codex' } });
    ...
    for (const inbox of [first, second]) expect(await inbox.nextJson(frame => frame.event === 'codex.update')).toMatchObject({ payload: { text: 'verbatim \n' } });
