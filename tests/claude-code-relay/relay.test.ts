...
import { ClaudeRelay } from '../../packages/bridge-runtime/src/claude-code/relay';
import type { ClaudeService } from '../../packages/bridge-runtime/src/claude-code/service';
...
it('pairs an isolated Claude Code deployment and routes requests, streams, offline errors and reconnects without an OpenClaw challenge', async () => {
  const state = await mkdtemp(join(tmpdir(), 'clawket-claude-code-relay-'));
  ...
  let registry: CompatWranglerDevProcess | undefined, relay: CompatWranglerDevProcess | undefined, owner: ClaudeRelay | undefined;
  const sockets: WebSocket[] = [];
  try {
    registry = await CompatWranglerDevProcess.start({ cwd: process.cwd(), configPath: 'apps/relay-registry/wrangler.claude-code.example.toml', port: registryPort, inspectorPort: inspector1, persistencePath: state,
    ...
    relay = await CompatWranglerDevProcess.start({ cwd: process.cwd(), configPath: 'apps/relay-worker/wrangler.claude-code.example.toml', port: relayPort, inspectorPort: inspector2, persistencePath: state,
    ...
    const pair = await post('/v1/pair/register', { displayName: 'Claude Code fixture', preferredRegion: 'us' });
    const qrPayload = JSON.stringify({ v: 2, k: 'cp', b: 'claude-code', s: registry.baseUrl, g: pair.gatewayId, a: pair.accessCode });
    ...
    class Service extends EventEmitter {
      conversation = this;
      async request(frame: any) { return frame.method === 'health' ? { backend: 'claude-code', protocol: 1 } : { marker: frame.params?.marker }; }
    }
    const service = new Service();
    owner = new ClaudeRelay(service as unknown as ClaudeService, { relayUrl, gatewayId: pair.gatewayId, relaySecret: pair.relaySecret, invitation: { ...invitation, codeKeyHex, qrPayload, attempts: 0 } }, () => {});
    ...
    const claim = await post('/v1/pair/claim', { gatewayId: pair.gatewayId, accessCode: pair.accessCode, clientLabel: 'Claude Code phone' });
    ...
    expect(await first.nextJson(frame => frame.id === 'one')).toMatchObject({ ok: true, payload: { backend: 'claude-code' } });
    ...
    for (const inbox of [first, second]) expect(await inbox.nextJson(frame => frame.event === 'claude-code.update')).toMatchObject({ payload: { text: 'verbatim \n' } });
