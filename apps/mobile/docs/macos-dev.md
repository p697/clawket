...
- Runs `pnpm install` in the app and `office-game`.
- Reapplies the local `expo-modules-core` Mac Catalyst patch if `pnpm install` overwrote it.
...
- If you change native iOS dependencies or manually edit the `ios` project, rerun `pnpm run dev:macos`. If Pods are still in sync but you want a clean refresh, run `FORCE_POD_INSTALL=1 pnpm run dev:macos`.
...
```bash
pnpm run dev:macos -- --clear
FORCE_POD_INSTALL=1 pnpm run dev:macos
METRO_PORT=8088 pnpm run dev:macos
MACOS_DERIVED_DATA_PATH=/tmp/clawket-mac-alt pnpm run dev:macos
```
...
pnpm run archive:macos
pnpm run export:macos
```
...
```bash
MACOS_ALLOW_SIGNING=1 pnpm run build:macos
MACOS_ALLOW_SIGNING=1 pnpm run archive:macos
```
...
- The current project still uses a local `expo-modules-core` Mac Catalyst patch. The scripts reapply it automatically after `pnpm install`, but this is still a temporary engineering workaround rather than a clean upstream fix.
- `export:macos` uses `ios/ExportOptions-macos-appstore.plist` and expects a signed archive from `MACOS_ALLOW_SIGNING=1 pnpm run archive:macos`.
