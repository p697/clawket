...
export class ClaudeCodeAdapter implements AgentAdapter {
  ...
  readonly capabilities = resolveCapabilities('claude-code', { attachments: false });
  ...
  connect(): Promise<void> {
    ...
    const ready = new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => finish(this.handshakeError ?? new AdapterError('timeout', 'Claude Code Bridge did not become ready')), 25_000);
  ...
  private receive(data: unknown): void {
    ...
    if (frame.type === 'res' && frame.id) {
      ...
      else pending.reject(new AdapterError(frame.error?.code === 'BRIDGE_UNAVAILABLE' ? 'bridge_offline' : 'server', frame.error?.code === 'BRIDGE_UNAVAILABLE' ? 'Claude Code Bridge is offline. Keep the Bridge running on your computer.' : frame.error?.message ?? 'Claude Code request failed'));
