...
export class PiAdapter implements AgentAdapter {
  ...
  readonly capabilities = resolveCapabilities('pi', { attachments: false });
  readonly questions = {
  ...
  connect(): Promise<void> {
    ...
    const ready = new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => finish(this.handshakeError ?? new AdapterError('timeout', 'Pi Bridge did not become ready')), 25_000);
  ...
  private rpc<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => { this.pending.delete(id); reject(new AdapterError('timeout', 'Pi request timed out')); }, method === 'models.select' ? 190_000 : 20_000);
  ...
  private receive(data: unknown): void {
    ...
    if (frame.type === 'res' && frame.id) {
      ...
      else pending.reject(new AdapterError(frame.error?.code === 'BRIDGE_UNAVAILABLE' ? 'bridge_offline' : 'server', frame.error?.code === 'BRIDGE_UNAVAILABLE' ? 'Pi Bridge is offline. Keep the Bridge running on your computer.' : frame.error?.message ?? 'Pi request failed'));
