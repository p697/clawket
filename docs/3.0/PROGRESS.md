...
- 2026-09-26 首页实时连接标识（负责人看过对比画布后批准推荐方案 A）：花名册的行来自两个及以上连接时，活动连接的每一行头像右下带 `good` 在线点（`AgentAvatar` `live`，与「需要你」红点同一个新 `StatusDot` 几何，锁 > 需要你 > 实时），只在活动连接就绪或 20 秒恢复宽限内显示，变为实时时 200 ms 淡入，读屏加「已连接」；单连接、离线、暂停、切换中不显示。「我的连接」与连接页不再把非活动连接写成「离线」，改为新文案「未连接」（19 语言），在线连接的平台图标带同一个绿点。数据：3.0 上线后 17% 的首页用户有多个连接，贡献 36% 浏览；6 个月代理口径 11% 的发送者贡献 49% 发送。按文件单进程验证 connection-details、Roster model、RosterPrimitives、RosterScreen、ConnectionsScreen、ConnectionScreen 共 81 项通过；未跑全量与 typecheck（本地资源规则），未提交、未真机验收。对比画布：https://claude.ai/artifact/9ik299XRvH3d22SQyVhJMj

- 2026-09-26 Claude 项目发现补验：负责人发现本地项目/对话不可见。此前测试配对为单项目，另发现首次默认后台配对添加 `--config` 后丢失 device 范围的真实缺陷，已修复并补父子启动回归；设备模式补充标准 `.claude.json` 已保存项目键，单项目授权仍不扩大。正常默认 Preview 设备配对实测发现 17 个项目、113 条原生会话，读取原生历史通过；安卓真机/iOS 模拟器均以正常设备配对展示原生会话和项目选择器，并分别在所选仓库/Chats 中新建聊天，真实回复确认 cwd；两端保留新的设备范围连接。CLI 6、catalog 4、saved-projects 1、service 7 针对性测试通过。此前“仅余两条 UI 流程”的结论不完整，以本轮补验为准。

- 2026-09-26 Claude Code 候选实现与实测：官方 CLI 2.1.280 + Agent SDK 0.3.283；已接独立 backend、设备/项目发现、原生只读历史与显式分支、私有会话/去重、真实模型、提问、审批、模型选择。安卓真机本地配对/对话、多选草稿强制退出后恢复通过；iOS 模拟器扫码/对话、自定义答案、真实文件允许/拒绝通过。实测修复了 onboarding 漏项、连续多选覆盖、审批快照缺少 session 包装导致重开会话卡住等问题；补充具体模型型号展示（原生 resolvedModel），双端截图复核、iOS Sonnet 真实切换、等待提问时停止及下一轮图片识别通过；过滤原生模型切换/中断操作记录，避免内部文本进入聊天。Preview 重启后继续会话保留上下文；原生 SDK 分支继承上下文且源记录不变。Claude 独立 Preview Registry/Relay 已部署并完成安卓六位码配对及真实回复；发布前 v1 五文件逐个串行 39 项通过，未部署其他后端或 Production，未发布包/App。原生 Desktop/TUI 仍不支持随意接管；现有原生会话只读，不能宣称完整远程桌面控制。规范化工具 ID 复用已补齐，iOS 重载后无重复工具卡；历史状态单文件 66 项、既有后端适配器回归、类型/设计/i18n/docs 检查通过。可开始限定范围的负责人验收；原生导入→分支完整 UI 导航和相册附件流程尚未覆盖，不将协议测试冒充这些路径已验收。详细边界和证据见 `docs/3.1/claude-code-qa-2026-09-26.md`。

- 2026-09-26 Codex 取消“主会话”：Bridge 启动及无 session 的模型列表读取不再生成落地聊天；旧首条记录保留 ID/历史，成为普通可删除聊天。新增可选 `entryMode: sessions`；移动端进入时恢复本设备该连接/Agent 上次仍存在的聊天，否则打开项目/会话列表，新建明确选择项目，删除当前聊天返回列表。保留 Bridge 自建聊天免费访问与原生历史既有权限；预览页改用“会话”入口；外部输入不再切换到空 main key。安卓真机与 iOS 模拟器完成首次进入、选项目新建、真实模型回复和返回恢复；安卓删除/重新进入、iOS 进程重启后恢复通过。OpenClaw/Hermes/Pi 原有聊天入口及历史在安卓复查正常（仅重新打开，本轮没有额外发送；开发版出现独立 PostHog flush 网络提示）。验证：移动全量 342 文件/3577 测试、Bridge Runtime 455、CLI 73、v1 replay 39、仓库类型检查及设计检查通过；required 首轮发现 ThreadScreen 新组件 mock 缺失，后续新增测试创建不稳定模拟 adapter 导致 OOM，均已修正。最终 required 的测试/设计项全过，唯一 i18n namespace 错误修复后 i18n 及其 8 项自测、最终 Thread/Entry 61 项、docs 门禁独立复测通过。详细证据见 `docs/3.1/codex-qa-2026-09-26.md`。仅刷新本地 QA Bridge/开发客户端，未发布。

- 2026-09-26 语音启动提速（负责人反馈按住后先看到“正在开启麦克风”，停顿约 0.5–1 s；先调研后授权落地调研中的第一、二步，明确不做边说边出字）。调研：与模型无关——“正在聆听”出现前不连网；PostHog `chat_voice_input_timing` 近 30 天 iOS native_started p50 246 ms（常态 180–350 ms），但负责人 iPhone 17 Pro 的两整段会话（09-21 14:44、09-26 12:46）每次 1.03–1.27 s，与回前台时长无关，现有埋点分不出是蓝牙/放音/录屏等环境；Android p50 99 ms、首个音频块 312 ms。再叠加 200 ms 长按门槛与点按松手才开始。根因：expo-audio 57.0.5 `AudioStream.start()` 每次切 `.record` + `.measurement`、强制 16 kHz 硬件采样率、激活会话、新建引擎，停止时同步关会话并切回播放类别；启动还和 Expo 全局串行 AsyncFunction 队列上的钥匙串/文件/PostHog 持久化互相排队（09-26 12:53 一次网关读凭证在麦克风启动完成 8 ms 后才返回）。另发现语音失败码埋点全被归成 `other`（已单独开任务，未在本轮修）。
  实现：新增本地原生模块 `apps/mobile/modules/clawket-voice-capture`（iOS Swift / Android Kotlin，无第三方依赖；Expo Audio 只保留权限接口）：独立串行队列；iOS 会话类别只配一次（`.playAndRecord` + `.measurement` + `.defaultToSpeaker`，不启用蓝牙输入），不强制硬件采样率、由 `AVAudioConverter` 转 16 kHz，录音期间允许触感；聚焦且在前台的会话页通过 `voiceCapture.hold()` 预建引擎（不开麦克风），录音间只暂停引擎；激活时若打断了其他 App 的声音则停止后立即释放，否则空闲 30 s、离开会话页或切后台释放；处理中断、路由/格式变化（自动重装 tap 续录）与媒体服务重置。Android 预建并复用 `AudioRecord`（仍用 MIC 音源，未改为 VOICE_RECOGNITION 以免改变已验收的识别输入），紧急音频优先级读线程 40 ms 读块。JS：麦克风按钮按下即开始，松手区分点击持续听写与按住发送，输入区仍长按 200 ms；删除 `authorizing` 阶段与“Preparing voice input…”文案（19 语言），权限已知时按下即进入聆听态并在此刻震动，手指未离开麦克风前不显示停止按钮；采集按 captureId 隔离，原生中断走状态事件后保留并转写已录音频。`chat_voice_input_timing` 的 native_started 增加 queue/activate/engine/start 分步毫秒、warm、端口类别和两个布尔值（只记类别不记设备名），同步 07 号文档与规格测试。规则同步到 `apps/mobile/AGENTS.md`、`22-voice-input.md`、`design-system.md`。
  验证：逐个 `--runInBand` 通过 useVoiceGesture 11、useChatVoiceInput 17（新增 4：按下即聆听并上报分步计时、只收本次 capture 的音频、原生中断后转写已录部分、仅聚焦且前台时预热）、ThreadPrimitives 34、ThreadView 104、analytics events 13；Mobile `tsc --noEmit`、`check:design-system`、`check:docs` 通过；`i18n-prune --strict` 唯一失败为并行会话新建 `ConversationEntry.tsx` 缺 `chat:Sessions`，本轮删键无遗漏/多余；ThreadScreen.test 加载即失败，来自并行会话未跟踪的 `ProjectPicker.tsx` 导入链。原生：`pod install` 仅新增该 pod，自动链接已注册模块；Pods 目标 `ClawketVoiceCapture` 模拟器 Debug 编译成功且无警告；Android `:clawket-voice-capture:compileDebugKotlin` 成功。未做整包 iOS/Android 构建、未装机、未真机验收，未部署、未提交；不含该模块的旧开发包在新 JS 下隐藏语音入口，需重新构建原生包（HT-VOICE-START-0926）。

- 2026-09-26 聊天时间线手感重做（负责人反馈：新对话首发“整屏一白、消息落到最底、上下跳一下”，要求像微信/Telegram 自上而下排列，并整体优化发送、回复、流式）。根因与修复：① 空态与首条消息是两棵树，首发时 FlashList 重挂载（首帧 `opacity: 0`）→ 现在同一个列表贯穿空态与首条消息，空态提示改为浮层并淡出；② `startRenderingFromBottom` 底部锚定，短对话每来一行都把上面的消息往上推 → 改为自上而下排列，进入线程仍用 `initialScrollIndex` + 超大 `viewOffset` 由原生夹紧到真正底部，且首帧在 FlashList 补齐上方行前保持透明（320 ms 兜底），避免进线程时上半屏空一两帧；③ 贴底跟随原先是 rAF + FlashList `scrollToEnd` 的 `setTimeout(0)`，新行/新文字会被裁在输入框下一两帧再跳上来 → 现在在 FlashList 的布局提交阶段直接调用原生滚动，追加的新行（发送气泡、思考占位、工具行）像 Telegram 一样平滑滑入，流式逐行增长、键盘/输入框变化与内容缩短保持同帧贴底；④ 流式结束时最后几个字没有淡入、iOS 行高晚一帧 → 结束后流式模式再保持 320 ms，并关闭会改写完整文本的 `remend` 转义（`- > 25`、`20~25`），结束瞬间不再变样式/跳高度；⑤ 每个流式分片都让所有可见行重渲染 → 行对象复用 + 稳定的行渲染器，分片只重渲染变化的那一行。同时修复排查出的回复生命周期闪烁：22 秒静默计时器在后端确认仍在运行时不再拆掉正在进行的回复（安静的工具/长思考）；同一会话重连不再用缓存快照替换可见消息；1 秒回复保持期按 render key 记忆，历史对账换 id 时上一条回复不再消失；工具分组按 render key 取 key；非本机发起的运行，占位与首个流式行共用同一身份，不再重挂载、重播入场。已验证：`tsc --noEmit`；按文件单进程运行 ThreadView（102）、model、toolGrouping、threadMessageEntrance、useStreamingSettleHold、useReplyEntranceDelay、useChatHistoryState（65）、useChatController queue（56）/adapter-events（23）/contract（53），并对超时器与重连两处修复做了“撤掉修复即失败”的反证；`ThreadScreen.test.tsx` 当前因另一会话新增的 `ThreadProjectContext` 引入在测试 mock 下无法加载，与本次改动无关。未做（下一轮候选）：传输中断时保留实时分段/工具行、外部运行文字早于其用户消息到达时的落位、≥3 分钟运行结束时插入时间分隔、手动重连时列表被加载态替换、键盘弹起改为 `KeyboardChatScrollView` 同帧上移、流式分片合并发布。待负责人真机验收；未提交。

- 2026-09-26 语音输入波形重做（负责人要求“简约高雅又哇塞”）：先用移植了 `speechLevel.ts` 的 HTML 实验台对比现状、中心涟漪、磁带滚动、镜像流、液态线五个方向，选定“涟漪 + 双路径”。新 `VoiceWaveform` 由 `voiceWaveformModel.ts` 驱动：UI 线程帧回调里 120Hz 固定步长弹簧跟随电平，每根条按距中心 30ms/根读取历史，音节从中心向两侧荡开并在边缘渐隐；最多 51 根 2.5pt 细胶囊，暗点常驻、亮条宽度随响度变化；开场从中心展开，安静时每 `Motion.voiceRipple`（2.6s）一次轻柔聆听脉冲，上滑取消改为压扁压暗；后台暂停，减少动态效果时只保留随声音变化的透明度。渲染从 25 个每帧提交的视图改为两条 SVG 路径，开场后每帧只更新一条。新增模型/组件测试 20 项；按新规则逐个 `--runInBand` 验证 voiceWaveformModel、VoiceWaveform、theme、ThreadPrimitives、ThreadView 共 160 项通过，mobile typecheck、design-system、docs 检查通过。同日两次卡死源于本会话以默认 9 个 worker 跑全量 Jest：已新增根 AGENTS.md「Local Test Resource Rule」（不开满、非负责人要求不跑全量、单文件串行），移动端 Jest 默认 `maxWorkers: 2`。那次全量中 3 个超时的套件单独复跑均通过，SessionPanel 的模块初始化失败来自并行会话在制品。未提交、未构建、未真机验收；输入框从 40pt 跳到 76pt 的高度变化未改动。
...
| HT-VOICE-START-0926 | 语音启动提速真机验收 | 装含 `clawket-voice-capture` 的新开发构建（iOS 需 pod install / `expo run:ios`）。iPhone 与 Android 上：按住麦克风立即说话，确认按下即出现聆听态与震动、开头的字不丢；点按持续听写、再点发送；输入区长按 200 ms 开始、轻点仍是打字；上滑取消有震动；连着 AirPods、正在放音乐时各录几次（音乐在录完后恢复）；切后台/来电后录音保留；离开会话页后无麦克风指示灯。之后在 PostHog 看 native_started 的 `start_ms` / `activate_ms` / `warm` / `input_route`，对照 09-26 之前的 ~250 ms 与 ~1.1 s 会话。 | 按下 100 ms 内有画面与震动；平时 native_started 明显低于旧值；开头字不丢；音乐录完即恢复；无残留指示灯。 | 待真机验收；本轮仅自动化与模块级编译 |
...
### 2026-09-25 — Move production speech to clawket.ai

Confirmed the published Google Play 3.0.0/30001 AAB embeds `wss://clawket-speech-preview.clawket.workers.dev/v1/speech`; Play now shows that version on the Production track at 100% rollout with no pending changes. EAS Production previously had no speech URL, while the local `.env.local` supplied the Preview URL to the signed build. Expo Updates is disabled in the existing Android/iOS native packages, so neither a Worker deploy nor an EAS environment edit changes installed 30001 clients.

Deployed the isolated `clawket-speech` Worker on Custom Domain `speech.clawket.ai`, with `workers_dev: false`, separate admission Durable Object and Worker secret binding. Active Cloudflare version: `76e3c31f-6a78-4920-a681-9170342b0672` (100%). The provider key value currently matches the existing Preview key, so Alibaba quota remains shared. Extended only zone machine-API WAF skip rule `74ad74cd271f4507966447c127784e87a` (ruleset `1a0b976fae77432d8182bf17741f2a4a`, version 10) to `/health` and `/v1/speech` on this exact host; rate limiting remains in effect. `GET /health` returns enabled/protocol 2; a 10-second synthetic PCM real WSS session returned a nonempty transcript. Globalping HTTP probes from Beijing, Shanghai and Guangzhou returned 200 on the new hostname (measurement `2fUuGB04X34TWXTWT00021CR4`); this does not certify every carrier or mobile WebSocket path.

Set EAS Production `EXPO_PUBLIC_SPEECH_URL=wss://speech.clawket.ai/v1/speech` and added the same value and a release guard to the EAS Production/TestFlight profile and local Android AAB script. Wired iOS Release validation into the generated Xcode environment as well as the EAS pre-install hook; Xcode dotenv loading now preserves an already supplied EAS/shell value. Both Xcode and EAS iOS checks failed with Preview and passed with Production in focused shell probes. Store builds now fail when the URL is missing or points to Preview. Development Preview remains available. Full `npm run check:required` passed before the final EAS-profile regression test and Xcode integration; the focused public-config suite passed 8/8 afterward, and `npm run check:docs` passed after the initial instruction updates. No privacy declaration/policy/settings edits. Owner stopped all packaging/release work and requires the fix to remain in 3.1.0 for owner-run iOS testing. Stopped the local Android build and removed the agent-created 3.0.1 worktree. No new package was uploaded or submitted to a store. Local Mobile `.env.local` now uses the production speech hostname so the owner’s iOS development build tests the corrected endpoint. The already published 30001 package has not changed.
...
### 2026-09-26 — Codex / Claude Code feasibility follow-up

Owner requested assessment only. Reviewed current official documentation, local Codex 0.153.3 generated schema and Claude Agent SDK 0.3.283 declarations. Non-inference probes passed Codex stdio initialize/project-filtered session list/model list, and SDK initialization/model discovery against installed Claude Code 2.1.280 with isolated dummy authentication. No production adapter changes, global upgrades, existing-session takeover or deployment. Recommendation and caveats (upstream experimental status, Claude authentication boundaries, shared approval semantics): `../research/codex-claude-integration-feasibility-2026-09-26.md`.


### 2026-09-26 — Codex implementation in progress

Owner requested product research, full independent Codex integration and iOS/Android interaction testing. Scope: `../3.1/codex.md`. Native 0.153.3 real-model text streaming completed in an isolated project; 0.157.0 initialization/model listing also verified without replacing the installed binary. Initial 15 protocol tests and 39 historical compatibility tests pass. Mobile typecheck passes. Device acceptance, broader gates and isolated Preview are in progress; this is not a release-readiness claim.

### 2026-09-26 — Codex implementation and device acceptance

Implemented an independent Codex App Server backend, project-scoped CLI lifecycle/pairing, isolated Registry/Relay, and the shared mobile chat/session/model/approval experience. Native histories remain read-only until explicit branching. Real Codex 0.153.3 and 0.157.0 pass text, model/reasoning, branching/history, steering, cancellation and restart checks. Android physical-device and iOS simulator flows pass; legacy OpenClaw, Hermes and Pi real-device conversations also pass. Fixed unmaterialized native thread recovery/rename, reasoning persistence, recovered message/tool duplication, pairing identity retention, local heartbeat cleanup and startup recovery after owner-lease conflicts plus a transient TLS failure.

Required gate passed (335 Mobile suites / 3,465 tests); final runtime 51 suites / 405 tests, 39 historical replay tests, isolated Relay integration, candidate build and docs/type checks passed. Isolated Codex Preview deployed under existing owner authorization; no Production service, npm package or signed/store distribution released. Detailed tested scope, deployment anchors, small-sample latency, Pi Preview QR limitation and remaining physical-iPhone/custom-provider/soak boundaries: `../3.1/codex-qa-2026-09-26.md`.

### 2026-09-26 — Codex project continuity direction study

Owner requested research/discussion before changing the Codex project/session model, and recommended a local remodex checkout. Cloned the reference outside this repository at `/Users/lucy/Desktop/me/references/remodex`, pinned `e0e342dac5cddd40db661bfcf76e5ab0e3913ef8`; reviewed project grouping, native history pagination, desktop IPC ownership/reconnect and regression scenarios. No competitor runtime was executed. Recommended scope and unresolved boundaries are in `../research/codex-project-sync-direction-2026-09-26.md`: one device connection, recent chats with a project filter, explicit cwd, and eventual original-thread desktop continuity; no Git/IDE scope. An interactive discussion mockup compares recent-first and project-first navigation. Current implementation/spec and all backend behavior remain unchanged; redesign awaits owner agreement.

### 2026-09-26 — Session Panel localized scheduled-run titles

Owner asked to localize the `Automation:` prefix in the Session Panel (Chinese UI). Evidence: OpenClaw 2026.9.1 writes `label = "Automation: <job name>"` for isolated cron sessions (`isolated-agent` dist); the App displayed it verbatim. The panel and Thread header now strip the Gateway prefix (also legacy `Cron:` / `[Cron]`) and render `Scheduled task: {{name}}` (zh-Hans `定时任务：{{name}}`) or `Scheduled task` when untitled; rename edits only the job name; search also matches the rendered title; row times use the localized relative-time copy (`7 分钟前` / `昨天`, previously `7m` / English `Yesterday`). Two new `common` keys in all 19 locales. Adapters and wire data unchanged; OpenClaw/Hermes paths unaffected beyond display. Uncommitted, awaiting owner device acceptance.

### 2026-09-26 — Add sheet opens without the skeleton flash

Owner report: with full photo access every open of the Thread Add sheet first showed four skeleton tiles, then swapped in the camera tile and photos while the `All photos` header button and the `Choose File` row appeared and pushed the menu down. Cause: a 320 ms `contentReady` gate always drew the skeleton, even with cached photos, and on the first open the permission read itself waited for the rise. Fix: `useRecentPhotos` is a process-wide `useSyncExternalStore` store — the mounted Thread reads photo access at once (never prompts), warms the 12-photo strip one second later while the sheet is closed (full access only: with a limited library the first PhotoKit read of a launch can raise iOS's selection alert — `PHPhotoLibraryPreventAutomaticLimitedAccessAlert` is not set — so that read still waits for a user-opened sheet), and a library-change subscription (after a resolved list) refreshes it before the next open, so a fresh screenshot no longer shifts the strip after the sheet rises; an unchanged refresh keeps the list identity. `ThreadAddSheet` resolves its media section through `resolveAddSheetMediaMode`: known access draws the final layout on the first frame, granted-but-loading keeps the strip (camera + placeholder tiles, header button, file row), skeleton only while access is unknown, photos that arrive after the strip is on screen fade in, and a dismissing sheet no longer morphs to the three tiles. Loads still stay outside the rise window (first-open 320 ms, per-open quiet refresh 960 ms). Android keeps the system Photo Picker (no reads). Mobile AGENTS and design-system recipe updated. Uncommitted, awaiting owner device acceptance.


### 2026-09-26 — Speech busy recovery after interrupted setup

Investigated production request `aa32d09f-75ab-4061-bd32-3911c7b6bcf9`: four busy rejections share an expiry around 04:51:55.9 UTC; the screenshot request at 04:49:53.787 UTC had 122110 ms remaining. The predecessor is not present in application logs, so its exact interruption cause remains unproven. Added red/green coverage for the concrete gap: device reservation completing after caller cancellation without protected cleanup.

Speech now uses a 25-second pending reservation, nonce-fenced promotion to the unchanged 150-second active allowance, protected bounded setup, request abort signals, and early authenticated v2 transport acceptance to observe cancellation during provider preparation. V1 HTTP semantics, provider task/audio ceilings, quotas and replay proofs remain intact; expired setup cannot revive, and old cleanup cannot free a replacement. Release has three bounded attempts and sanitized reservation/release diagnostics. Matching ignored operator configs carry the abort flag; no live config was changed.

Mobile retries only explicit pre-ready busy rejections, up to eight retries started within 30 seconds, with cancellable backoff and local recording retained. It never automatically replays an accepted task/lost result or retries auth/quota/network failures. Cancellation closes late transports, and delayed alerts calculate remaining cooldown. Version remains 3.1.0.

Validation: `npm run check:required` passed (337 Mobile suites / 3496 tests), 39 v1 replay tests passed, Speech 42 unit tests and four self-contained real-workerd integration tests passed; focused Mobile speech coverage passed 38 tests. Runtime tests cover disconnect during provider upgrade followed by another same-device recording, consecutive v1/v2 results, active-session exclusion, orphan expiry and stale activation/release fencing. Fake-clock tests retain 110-second tasks; existing ten-minute checkpoint/recovery and both OpenClaw/Hermes voice flows pass. Local runtime fixture uses supported compatibility date 2026-09-17 with explicit abort flag; service configs retain 2026-09-19. Evidence: `/tmp/clawket-speech-{required,compat,runtime,final-unit,mobile-tests}.log`.

No private audio accessed, Production/Preview deployed, distribution built, version bumped or store/OTA published. Server cleanup requires an authorized service deployment; mobile backoff/cooldown improvements require the updated App build. Actual provider/device/network acceptance is not claimed from the synthetic local fixture. Existing concurrent Codex and UI work preserved.


### 2026-09-26 — Codex continuity direction approved; user-input audit

Owner approved the proposed project/continuity direction and asked explicitly about ask-user-question support. Local remodex source confirms structured question UI, question-ID answer aggregation and desktop follower reply routing. Clawket currently has basic owned-App-Server question handling, but combined options/custom input and desktop-owned routing are incomplete; prior device QA is not a full native user-input acceptance claim. Recorded required end-to-end scenarios, distinct dismissal/cancellation and approval-bearing prompt semantics in the direction document and the approved-next-phase section of `../3.1/codex.md`. No runtime changes or release in this audit.


### 2026-09-26 — Authorized speech busy fix deployed and verified

Owner explicitly requested completing deployment. Deployed only speech Preview and Production, in that order: `a47225c3-a39d-4775-ab1f-e4d3a7bbe6f3` at 05:54:10 UTC and `491efc20-7a1c-4f49-9ad8-9706f500dad7` at 05:57:12 UTC, both 100%. Previous rollback anchors: Preview `ddeaf1b0-cb86-44e7-a639-f920347d2df4`, Production `76e3c31f-6a78-4920-a681-9170342b0672`. Existing distinct admission namespaces and secret bindings verified unchanged. Request abort enabled; invocation logging/traces stay disabled and query redaction is now explicitly enabled in both cloud and operator/template configs. DNS, WAF, quotas and provider secrets were not changed.

Both remote entrypoint exports match the verified local bundle SHA-256 `2cc0ba29057e5042af3060413da3ae7b702fc4effee5312453ccf3297174966a`. Speech typecheck, 42 unit tests, four real-workerd tests and dry-runs passed; the preceding full required gate and 39 v1 replay tests remain green. Each deployed environment passed five real Alibaba synthetic-PCM transcriptions: v1, 30-second v2, immediate consecutive v2, and recovery after pre-ready cancel and abrupt disconnect. Both interruption scenarios recovered on the first subsequent connection (zero busy retries); the production recovery recordings completed in 4.31/4.49 seconds including three seconds of audio. Deliberate overlapping requests were rejected while the original recording completed. Cloud logs verify new-version execution and lease release.

Evidence and per-file source hashes: local `evidence/speech-busy-0926/verified-deployment.json`, deploy logs, cloud probe logs, unit/runtime logs. Only synthetic audio used, no private recording read. This is cloud endpoint acceptance, not physical-device/all-carrier/ten-minute provider acceptance. Existing installed Apps receive server-side cleanup immediately; client backoff/cooldown changes remain in the 3.1.0 source pending an owner build. No App distribution, OTA, npm publication, Relay/Registry/Bridge deployment or version bump.

### 2026-09-26 — Codex continuity implementation and native questions, acceptance in progress

Implemented device-scoped project discovery (including current Desktop named local projects), paginated native history, explicit project selection, native owner IPC routing, grouped question IDs/options/custom answers, and durable bounded phone drafts. Real installed Codex 0.153.3 Plan mode produced two native questions: iOS LAN answered an owned conversation; Android Preview answered the original conversation through a second independent Bridge using Desktop IPC. Both original runs completed and followers received resolution/terminal events. Android cold launch restored an exact custom draft and the previous option selection. No current user-owned task was mutated.

Owner requested a clearer selection UI. Replaced plain option text with explicit radio cards, neutral selected fill/ink outline/check, on-demand custom input, localized step navigation, a quiet chat question row and project context. Inspected selected/unselected screenshots on iOS simulator and physical Samsung Android. Seven focused question/draft tests and 57 Codex runtime tests pass; historical replay 39 passes. Full required checks are running after fixing the new capability expectation and the ThreadScreen test's missing project-sheet mock. One accidentally broad local runtime invocation included unavailable external Hermes/Codex integration prerequisites and failed; these are not device acceptance results.

Remaining acceptance includes final cancellation/cross-device dismissal, project/cwd creation after the latest discovery update, native settings/ownership edge cases and the current candidate's legacy-backend smoke. Actual Codex Desktop UI could not be automated because the computer-use tool rejects that app; real IPC routing between independent Bridge owners is tested, but it is not a claim of full Desktop UI acceptance. Evidence is in the private local directory `~/.clawket/testing/codex-continuity/`. No Production deployment, package publication, version bump or distribution build.

Codex continuity follow-up: `check:required` passed (341 Mobile suites / 3,553 tests; 420 CI-safe runtime tests). Android Preview cancellation produced native `run_finished cancelled` and automatically dismissed the open iOS question sheet. Android project picker selected the existing Clawket repository; the newly created conversation ran `pwd` and returned the selected `/Users/lucy/Desktop/me/clawket` cwd. Device-mode recent chats now sort by human recency (legacy main-first order unchanged), and project context is emitted immediately on creation. Focused project/cache tests pass 51; adapter tests pass 8. CLI rejects conflicting device/project scopes instead of silently widening saved pairing; five focused CLI tests pass. A delayed native IPC handler response is now fenced to its original socket/client identity. Final focused checks are continuing; no release performed.

Codex question UI follow-up: latest screenshots show explicit radio option cards on both platforms (`ios-radio-final.png`, `android-radio-selected.png`). Current Android OpenClaw and Hermes connections both restored history and returned the exact regression markers. Both Codex QA Bridges were rebuilt/restarted; LAN real inference and idempotent retry passed again. The first LAN restart reported an existing owner and left that owner serving; an explicit second restart succeeded and the new process was verified. The initial failed attempt is retained, not counted as a pass or attributed to a proven root cause. The previous Pi temporary QA runtime/configuration is absent; replacement isolated Preview QR pairing and a real Qwen 3.5 Plus reply passed on the physical Android, using durable Clawket-owned QA storage. Full Codex continuity acceptance remains in progress as recorded in the updated QA document.

### 2026-09-26 — Project path moves into the Thread header

Owner found the directory row above the composer intrusive, then selected the existing header subtitle as its home. Removed the dedicated row/component; idle project conversations show the full path with single-line middle truncation in the fixed-height header pill. Working dots and reconnect/offline states take precedence; the path returns on idle. Full-path accessibility hint, session-row project names and explicit new-chat project selection remain available. No backend branching or transport behavior changed. Focused Thread tests passed 177, preceding Thread/SessionPanel tests passed 68, Mobile typecheck and docs passed. Physical Android screenshot confirms the path below the title without additional composer/header height. Design checks passed; iOS simulator screenshot also confirms the same fixed-height path subtitle and restored composer space. Existing Codex continuity acceptance remains separate from this UI adjustment.

### 2026-09-26 — Compact project session rows

Owner requested removing the three-line title/project/preview hierarchy and the misleading group-chat icon in Codex's session sheet. Project rows now share project name and preview on one secondary line under title/time, with independent truncation and a 40% cap for the project fragment when a preview exists. Filtering to one project suppresses repeated row project labels. Project-backed direct/other conversations use the Terminal glyph; main avatars, genuine group/channel kinds, unread/attention signals and session actions remain unchanged. Existing SessionPanel/model tests passed 33, Mobile typecheck, design-system and documentation checks passed. iOS simulator screenshots verify both the compact all-projects list and the filtered list with project names omitted; the actual project picker selection also passed. No runtime, cloud or release changes.

### 2026-09-26 — Live Codex model catalog and native order

Owner reported only five models. Read-only native probes confirmed the configured PATH CLI 0.153.3 returns five picker-visible models and no pagination; Desktop's bundled 0.158.0-alpha.2 returns seven, adding GPT-6-Sol/Luna. Hidden-model requests did not explain the difference. Mobile already queries on every picker open. Added bounded native model pagination with malformed/cyclic-page rejection and accepted supported SemVer prerelease/build suffixes. Only the two private QA Bridge configs now explicitly use the desktop-bundled executable; global CLI and user credentials were not changed. The first Preview restart failed with an existing-owner message; a subsequent explicit restart succeeded and both authenticated catalogs now return seven models.

Owner then requested newest models first. Codex adapter now preserves native recommendation order via optional `ModelInfo.sortOrder` on list and settings replies; the shared picker honors it through provider grouping/search. Other backends keep alphabetical ordering. Current native order is GPT-6-Astra, GPT-6-Sol, GPT-6-Luna, then the 5.6 family and 5.5; no model names or release order are hardcoded.

Verified: 72 Codex runtime tests, 40 mobile picker/adapter tests, runtime/mobile typechecks, Bridge build, design and docs checks, 39 historical v1 replay tests. Both added models selected successfully and returned exact real inference markers in a newly owned QA conversation. iOS simulator and physical Android Preview show all seven models. A separate iOS selection attempt in an imported history conversation timed out waiting for Desktop settings acknowledgement; it was not replayed or counted as a pass. That native-owner settings boundary remains an acceptance limitation, distinct from successful catalog/new-owned-chat tests. No production deployment, distribution or package publication.


### 2026-09-26 — Claude Code Desktop / CLI feasibility research

Owner assigned this fork to research Claude Code and explicitly included both Desktop and CLI. Reviewed current official CLI 2.1.283 / SDK 0.3.283, inspected installed CLI 2.1.280 and Desktop engine 2.1.281, and read Happy/Happier/Hapi/Paseo implementations. Official SDK read-only discovery returned 111 non-programmatic sessions, including 110 Desktop transcripts across six directories; project-filtered listing and history reads passed. Official `claude agents --json --all` returned three live Desktop sessions with matching native IDs and idle/busy state. Desktop metadata mapped 107 records to native transcripts. Actual Desktop UI was inspected without sending to or taking over existing sessions.

Projects/history are feasible for both sources; resuming a released session is different from controlling an existing Desktop/TUI owner. Report recommends a shared device/project experience, SDK-owned execution, explicit ownership and a continuity-first proof before committing to seamless native takeover. Channels remains a preview supplement. Independent CLI is not authenticated; no real model, phone, owner handoff or approval round-trip is claimed. No implementation, dependency change, global upgrade, test suite, deployment or release. Documentation checker passed (7 instruction pairs). Findings, source revisions, aggregate probe evidence and authentication follow-up: `../research/claude-code-desktop-cli-feasibility-2026-09-26.md`. Concurrent Codex and UI work preserved.
