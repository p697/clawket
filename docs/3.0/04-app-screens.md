...
2026-09-22 配对 prompt（负责人要求精简）：保持接近原长度，合为一段，仅保留确切命令、开源 CLI 用途、用户授权在当前对话回复实际打印的临时配对码，以及只发码和后端名称、不附带其他凭据。19 语言同步，Production / Preview 命令及配对协议不变；不保证外部 Agent 接受。
...
4. 列表：每个 Agent 一行；用户置顶的会话作为带 📌 的行紧跟其 Agent 之后。排序（2026-09-16 负责人决定，替代原「需要你 > 有未读 > 最近活动」）：Agent 级手动置顶 > 最近一次有人参与的活动时间降序；2026-09-22 负责人授权优化：跨连接全局排序，不再强制同连接相邻；置顶在整个列表生效，时间相同按稳定连接 / Agent ID 兜底。「有人参与的活动」= 主会话 / 直聊 / 群 / 渠道会话（`HUMAN_SESSION_KINDS`）里最近的用户消息或面向用户的回复：OpenClaw 取 Gateway 的 `max(lastInteractionAt, lastActivityAt)`（心跳轮询与元数据 patch 不推高它，`updatedAt` 会），Hermes 优先取独立 `lastActivityAt`（用户 / 有文本的助手消息时间），旧 Bridge 回退 `updatedAt`；子 Agent 与定时任务会话不计入。未读与「需要你」只做徽标，不参与排序——它们是瞬态，会让行在用户没动的情况下上下跳；1–4 行的列表里徽标一眼可见，置顶不省时间。行右侧时间与未读水位线用同一个活动时钟，缓存态与在线态顺序一致。
5. 空态（有连接但无 Agent，理论上不会）：「这个连接上还没有 Agent」。

**Agent 行**：`AgentAvatar`（56，状态环）+ 名字（body 600）+ 预览（secondary，一行）+ 右侧：活动时间（caption），未读 / 需要你 / 锁定标记显示在其下方，不再替代时间。置顶 Agent 名旁显示 Pin。预览与时间同源，非主会话预览带会话来源；点击 Agent 仍打开 main。就这三样，不显示连接名、后端名、传输方式。缓存态：右侧仍显示缓存消息的活动时间，不能切换为同步时间；上次同步时间仅作独立无障碍信息。锁定态（免费用户的非 main Agent）：头像去饱和，时间下方锁图标。实时标记（2026-09-26 负责人决定）：花名册的行来自两个及以上连接时，活动连接的每一行（含置顶会话行）头像右下带 `good` 在线点（`AgentAvatar` `live`），告诉用户哪些行是实时的、点其他行会切换连接；只在活动连接就绪时出现，20 秒恢复宽限内保持不动（此时行虽来自缓存也不闪），离线、暂停、切换中不显示；只有一个连接时不显示；同一角的优先级：锁 > 需要你 > 实时；读屏在名字后加「已连接」。不做行高亮、阴影或按连接分组（活动连接的行按全局活动散开，灰底已是按下态与 iPad 选中态）。
...
OpenClaw 通过 `cron.list` 返回的系统监控任务（`heartbeat`、`skillCollectionReview`）保留在任务列表和运行记录中，但列表标明由 OpenClaw 管理，不提供独立开关。详情改为只读，展示任务名、时间规则、状态与运行记录，并说明不能在 Clawket 编辑或删除；不显示空白提示词、保存、立即运行或删除。按 payload 类型识别，不靠任务名；普通 OpenClaw 与 Hermes 任务仍走原有编辑流程。
...
分组：Pro（状态 / 横幅 / 恢复购买）；连接（「我的连接」列表：label、Agent 名、状态——只有活动连接会是在线 / 正在连接 / 离线，其余连接写「未连接」，暂停的写「已暂停」（2026-09-26：非活动连接此前一律误写「离线」）；在线连接的平台图标带与花名册相同的绿点；行左滑 → 「暂停 / 恢复」「移除」，与连接页同样二次确认；点行 → 连接页；「添加连接」→ 引导模态；免费第 2 个连接 → 付费墙 `gatewayConnections`）；外观（主题、强调色、聊天外观、App 图标 Pro）；帮助（帮助中心、OpenClaw 文档、Hermes 文档、发布说明、OpenClaw Releases、反馈）；社区（分享、评分、Discord）；关于（版本、开源仓库、隐私、条款）；开发者（Debug 模式、Preview 环境、设计系统、清缓存、重置设备）。

**连接页 `Connection`**（2026-09-17 负责人定稿，唯一的连接级页面；花名册「管理」、我的连接列表、Agent 资料页「连接」行都指向这里）：头部 = 官方图标 + 连接名 + 状态（与「我的连接」同一套，`resolveConnectionPresence`）+ Agent 名；主操作「重新连接 / 恢复连接」+「暂停此连接」（确认）；「名称」行 → 改名弹层（`RenameSheet`，只改本机 label，不动凭据、不断 OpenClaw 连接；Hermes 无 Bridge 名与本地模型的 Agent 名来自连接名，改名立即镜像到花名册与缓存，活动连接重新握手一次）；「详情」分组只读：后端、传输、环境、服务器地址、Bridge 版本、Bridge 能力、最近就绪；免费用户多一组「免费连接 · 当前」或「设为免费连接」（24 小时冷却尾值）；底部「移除连接」（确认）。原「高级设置」子页、账户设置的连接分段和 Agent 设置的连接分段一并删除，不再有第二处连接详情或生命周期入口。
