...
**Agent 行**：`AgentAvatar`（56，状态环）+ 名字（body 600）+ 预览（secondary，一行）+ 右侧：活动时间（caption），未读 / 需要你 / 锁定标记显示在其下方，不再替代时间。置顶 Agent 名旁显示 Pin。预览与时间同源，非主会话预览带会话来源；点击 Agent 仍打开 main。就这三样，不显示连接名、后端名、传输方式。缓存态：右侧仍显示缓存消息的活动时间，不能切换为同步时间；上次同步时间仅作独立无障碍信息。锁定态（免费用户的非 main Agent）：头像去饱和，时间下方锁图标。
