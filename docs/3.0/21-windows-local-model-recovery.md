...
Stop acknowledges only after the owned child exits. Start waits for an already-stopping supervisor to release ownership, then confirms the replacement control endpoint. Both operations have bounded deadlines and preserve exclusive ownership. Every installation attempt uses a new release directory; failed dependency installs cannot mutate active or rollback snapshots. The installed CLI must pass `--help` before its manifest is activated. Windows CI injects a partial npm installation failure and verifies both manifests and the old bundle remain unchanged.
