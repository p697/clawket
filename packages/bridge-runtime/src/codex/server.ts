...
export class CodexServer {
  ...
  constructor(readonly conversation: CodexService, private readonly token: string, private readonly log: (message: string) => void = () => {}) {
  ...
  private accept(socket: WebSocket): void {
    ...
    socket.on('message', raw => {
      ...
      void (async () => {
        ...
        try {
          const frame = JSON.parse(raw.toString()) as CodexRequest;
          ...
          if (socket.readyState === WebSocket.OPEN) this.send(socket, { type: 'res', id, ok: false, error: { code: 'codex_error', message: error instanceof Error ? error.message : 'Codex request failed' } });
        } finally { pending--; }
      })();
    });
  }
