...
export interface ClaudeRelayConfig { registryUrl?: string; relayUrl: string; gatewayId: string; relaySecret: string; invitation?: ClaudeInvitation }
...
export class ClaudeRelay {
  ...
  private readonly update = (payload: unknown) => this.send(JSON.stringify({ type: 'event', event: 'claude-code.update', payload }));

  constructor(private readonly service: ClaudeService, private readonly config: ClaudeRelayConfig,
    private readonly persistInvitation: (invitation: ClaudeInvitation) => void,
  ...
  private connect(): void {
    ...
    socket.on('open', () => {
      ...
      this.log('claude-code relay transport connected');
    ...
    socket.on('message', raw => {
      ...
      if (text.startsWith(PREFIX)) {
        try {
          ...
          if (control.event === 'relay.ready') {
            ...
            this.log('claude-code relay ready');
      ...
      void (async () => {
        ...
        try {
          const frame = JSON.parse(text) as ClaudeRequest;
          ...
          if (id && this.socket === socket) this.send(JSON.stringify({ type: 'res', id, ok: false, error: { code: 'claude-code_error', message: error instanceof ClaudeFault ? error.message : 'Claude request failed. Check the local Bridge configuration.' } }));
    ...
    socket.on('error', (error: Error & { code?: string }) => {
      ...
      this.log(`claude-code relay transport error code=${code}`);
    });
