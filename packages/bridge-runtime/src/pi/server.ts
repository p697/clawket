...
export class PiServer {
  ...
  private readonly service: PiService;
  private readonly update = (value: unknown) => {
    const data = JSON.stringify({ type: 'event', event: 'pi.update', payload: value });
  ...
  constructor(readonly conversation: PiService, private readonly token: string, private readonly log: (message: string) => void = () => {}) {
    if (Buffer.byteLength(token) < 32) throw new Error('Bridge token must contain at least 32 bytes');
    this.service = conversation;
  }

  async start(port = 17881, host = '127.0.0.1'): Promise<number> {
  ...
  private accept(socket: WebSocket): void {
    ...
    socket.on('error', error => { if (isWebSocketMaxPayloadError(error)) this.log('pi client rejected code=frame_too_large source=ws_max_payload'); });
    ...
    socket.on('message', raw => {
      ...
      void (async () => {
        ...
        try {
          const frame = JSON.parse(raw.toString()) as PiRequest;
          ...
          if (socket.readyState === WebSocket.OPEN) this.send(socket, { type: 'res', id, ok: false, error: { code: 'pi_error', message: error instanceof Error ? error.message : 'Pi request failed' } });
        } finally { pending--; }
      })();
    });
  }
